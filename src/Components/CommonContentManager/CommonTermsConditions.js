...
function CommonTermsConditions() {
	...
	const getTermsConditionsDetails = async () => {
		try {
			setIsLoading(true);
			let resp = await FetchApi(Endpoints.getTermsConditions);
			if (resp && resp.status) {
				setData(resp);
				setIsLoading(false);
			}
				...
				<div className='main py-5'>
					<div className='container'>
						<div className='about-section'>
							<h1 className='text-gray1 fs-30 medium pb-3 fs-30'>{t("Terms and Conditions")}
							</h1>
...
export default CommonTermsConditions;
